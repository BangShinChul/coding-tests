...
public class KakaoQuizDollCrane {
  ...
  private void solution1(int[][] board, int[] moves) {
    Stack<Integer> stack = new Stack<>();
    int popDollCount = 0;

    for (int move : moves) {
      for (int[] vertical : board) {
        int doll = vertical[move - 1];
        if (vertical[move - 1] != 0) {
          if(!stack.isEmpty() && vertical[move - 1] == stack.lastElement()) {
            vertical[move - 1] = 0;
            stack.pop();
            popDollCount += 2;
          } else {
            stack.push(vertical[move - 1]);
            vertical[move - 1] = 0;
          }
          break;
        }
    ...
    System.out.println(popDollCount);
  }
  ...
  public static void main(String[] args) {
    ...
    KakaoQuizDollCrane main = new KakaoQuizDollCrane();
    main.solution1(board, moves);
  }
}
/**
