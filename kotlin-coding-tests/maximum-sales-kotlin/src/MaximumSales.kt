...
// sliding window, 오답
fun solution1(N: Int, K: Int, sales: Array<Int>) {
    var result = 0
    ...
    for (i in K until N) {
        val sum = result + (sales[i] - sales[i-K])
...
// sliding window, 정답
fun solution2(N: Int, K: Int, sales: Array<Int>) {
    var result = 0
    var sum = 0
...
fun main(args: Array<String>) {
    ...
    val N = sc.nextInt() // 매출액 데이터 개수
    val K = sc.nextInt() // 연속된 일 수
    sc.nextLine()
    val sales = sc.nextLine().split(" ").map { it.toInt() }.toTypedArray() // 매출액 데이터 목록
